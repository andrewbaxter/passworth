    ...
    loop {
        ...
        match (|| {
            ...
            if version < 0i64 {
                {
                    let query =
                        "create table \"values\" ( \"path\" text not null , \"points\" text not null , \"rev_stamp\" text not null )";
...
pub fn values_insert(
    ...
    let query = "insert into \"values\" ( \"rev_stamp\" , \"path\" , \"points\" ) values ( $1 , $2 , $3 )";
    db.execute(query, rusqlite::params![stamp.to_rfc3339(), path, value]).to_good_error_query(query)?;
    Ok(())
}
...
pub fn values_get(db: &rusqlite::Connection, prefix: &str, at: i64) -> Result<Vec<DbRes1>, GoodError> {
    let mut out = vec![];
    let query =
        "select max ( \"values\" . \"rowid\" ) as \"rev_id\" , \"values\" . \"rev_stamp\" , \"values\" . \"path\" , \"values\" . \"points\" from \"values\" where ( ( \"values\" . \"path\" like $1 ) and ( \"values\" . \"rowid\" < $2 ) ) group by \"values\" . \"path\"";
